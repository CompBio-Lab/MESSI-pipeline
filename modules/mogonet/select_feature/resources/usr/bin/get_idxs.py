...
def get_idxs(mdata, random_state, block_num, test_size):
    ...
    X = ann.to_df().rename_axis('sample_names').reset_index() # This is single "block"
    Y = ann.obs.reset_index(drop=True)
    df = pd.merge(X, Y, on="sample_names")
