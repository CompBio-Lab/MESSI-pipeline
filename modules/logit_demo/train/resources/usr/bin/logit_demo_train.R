...
# Script to train logit_demo model
doc <- "This script is to run logit_demo method, run it only against the train MAE from previously
processed data. Output is the path to this trained model and the test MAE.

Usage:
  logit_demo_train.R [options]
...
Options:
  ...
  --method_name=METHOD_NAME   Name of the method [default: logit_demo]
"
...
main <- function(fold_path, label, prefix, method_name) {
  ...
  # Given we using glm here, we do another layer of preprocessing for both train and test data
