...
Options:
  ...
  --method_name=METHOD_NAME   Method name input from upstream [default: demo_logit]
...
main <- function(model_path, test_path, label, output_ext, method) {
  ...
  pattern <- "-fold.*"
  result_table <- tibble(sample_name = test_data$sample_name,
              y = test_data$y) %>%
        full_join(probs_df,  by = "sample_name") %>%
