...
process INTEGRAO_TRAIN {
	...
	*/
	output:
		// It generates two model, so have to explicitly set pattern here
		// Output the model directory and the preprocessed data dir for predict step, and log file for debugging
		tuple val(dataset_name), val(fold_path.name), path("${fold_path.name}-model"),	emit: model_dir
		tuple val(dataset_name), val(fold_path.name), path(fold_path),									emit: preprocessed_data_dir
		// Due to design, integrao requires full data rather just test data only, so we directly use the fold_path as preprocessed data dir for predict step
