...
def extract_dataset_name(label):
    dataset_name = label.rsplit('-', 1)[0]
    return dataset_name

def generate_result_table(predicted, meta_df, method_name, label, decimals=3):
    # First read in the previous metadata table
    df = meta_df.copy()
    # Do some renaming to columns
    df = df.rename(columns={"response": "y"})
    # Append the method name, predicted probabilities, and dataset name
