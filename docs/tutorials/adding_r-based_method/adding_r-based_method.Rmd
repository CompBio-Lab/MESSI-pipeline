...
#### Command Line Script in R using docopt for preprocessing

Given these are high dimensional data with n << p, hence we taking randomly 25 predictors on each omics. And, we will provide the data tranposed back as n x p_i format.
...
The input of this training step is being transformed under nextflow's operators like `join` and `multiMap` in the subworkflow in the following format:

- First arg is dataset name
- Second arg is the full mae path
- Third arg is the fold path (The earlier directories that we created after splitting the MAE)

Thefore, we follow a similar step earlier to create a nextflow script and its corresponding cli script.

#### Nextflow train script

We could again use a template at `~/templates/nxf_scripts/method_train.nf` and create the train nextflow script at `~/modules/logistic_reg/train/main.nf`. Some important things to do:

- Declare the apptainer image used and the docker URI
- Implement the non-interactive R script that hold actual logic of training model using the training data
  - Write out the test data, so that its passed to downstream process use
  - Change the cli script to executable permission with `chmod +x`
...
After having the nextflow script ready, we could then proceed to the non-interactive cli R script that handles training logic of our data per fold.


#### Command Line Script in R using docopt for training


Now, the main input of the script is `fold_path`, which is the directories of folds we created earlier of the following format:

```bash
fold_path1/
 |_________ fold_path1_train_mae/
 |_________ fold_path1_test_mae/
...
fold_pathK/
```

We need to load those training portions and apply logistic regression on it. Given these are high dimensional data with n << p, hence we taking randomly 25 predictors on each omics, and made sure this is applied to both 

We could use the template at `~/templates/cli_scripts/r_cli.R` to start off and write out the rest logics