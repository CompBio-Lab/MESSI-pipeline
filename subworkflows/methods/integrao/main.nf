...
def method_dir = "${modulesDir}/integrao"
include { INTEGRAO_PREPROCESS }   from "${method_dir}/preprocess"
include { INTEGRAO_TRAIN }        from "${method_dir}/train"
include { INTEGRAO_PREDICT }      from "${method_dir}/predict"
// This one could be optional and not implement for now
// so make sure to leave it comment out
// include { INTEGRAO_SELECT_FEATURE }      from "${method_dir}}/select_feature" 
  ...
  main:
    // This if here is just to quickly disable a run of method
      ...
      */
      ...
      INTEGRAO_PREPROCESS ( data_copy )
      // Then join the original copy with actual folds after split
      data_copy.join(  INTEGRAO_PREPROCESS.out.fold_splits, by:0 )
              .multiMap { it ->
      ...
      */
      ...
      /*
        ...
        fold data. 
        NOTE: integrao uses full data instead of test data, the input is from preprocess step
        # The data is contained in the model output so directly use it
      */

      INTEGRAO_PREDICT (  
        INTEGRAO_TRAIN.out.model_dir, // directly use the full model dir that contains unsup and sup model + hparams json
        INTEGRAO_TRAIN.out.preprocessed_data_dir, // directly use the preprocessed data from train since integrao uses full data instead of test data
      ...
      */
      ...
      INTEGRAO_PREDICT.out.result_table
                    .groupTuple(by: 2)
