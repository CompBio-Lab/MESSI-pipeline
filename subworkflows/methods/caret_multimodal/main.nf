...
def method_dir = "${modulesDir}/caret_multimodal"
include { CARET_MULTIMODAL_PREPROCESS }   from "${method_dir}/preprocess"
include { CARET_MULTIMODAL_TRAIN }        from "${method_dir}/train"
include { CARET_MULTIMODAL_PREDICT }      from "${method_dir}/predict"
// This one could be optional and not implement for now
// so make sure to leave it comment out
// include { CARET_MULTIMODAL_SELECT_FEATURE }      from "${method_dir}}/select_feature" 
...
def method_name = "caret_multimodal"
def saveMode = "method"


workflow CARET_MULTIMODAL {
  take:
  ...
  main:
    // This if here is just to quickly disable a run of method
      ...
      */
      ...
      CARET_MULTIMODAL_PREPROCESS ( data_copy )
      // Then join the original copy with actual folds after split
      data_copy.join(  CARET_MULTIMODAL_PREPROCESS.out.fold_splits, by:0 )
              .multiMap { it ->
      ...
      */
      ...
      CARET_MULTIMODAL_TRAIN ( train_input )

      // Do some transformation to make a multiMap that has two branches for predict
      CARET_MULTIMODAL_TRAIN.out.model
                  .join(CARET_MULTIMODAL_TRAIN.out.test_data, by: [0, 1])
                  .multiMap { it ->
      ...
      */
      ...
      CARET_MULTIMODAL_PREDICT.out.result_table
                    .groupTuple(by: 2)
