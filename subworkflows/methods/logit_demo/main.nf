...
def method_dir = "${modulesDir}/logit_demo"
include { LOGIT_DEMO_PREPROCESS }   from "${method_dir}/preprocess"
include { LOGIT_DEMO_TRAIN }        from "${method_dir}/train"
include { LOGIT_DEMO_PREDICT }      from "${method_dir}/predict"
// This one could be optional and not implement for now
// so make sure to leave it comment out
// include { LOGIT_DEMO_SELECT_FEATURE }      from "${method_dir}}/select_feature" 
  ...
  main:
    // This if here is just to quickly disable a run of method
      ...
      */
      ...
      LOGIT_DEMO_PREPROCESS ( data_copy )
      // Then join the original copy with actual folds after split
      data_copy.join(  LOGIT_DEMO_PREPROCESS.out.fold_splits, by:0 )
              .multiMap { it ->
      ...
      */
      ...
      LOGIT_DEMO_TRAIN ( train_input )

      // Do some transformation to make a multiMap that has two branches for predict
      LOGIT_DEMO_TRAIN.out.model
                  .join(LOGIT_DEMO_TRAIN.out.test_data, by: [0, 1])
                  .multiMap { it ->
      ...
      */
      ...
      LOGIT_DEMO_PREDICT.out.result_table
                    .groupTuple(by: 2)
