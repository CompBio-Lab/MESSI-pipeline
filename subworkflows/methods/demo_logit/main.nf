...
def method_dir = "${modulesDir}/demo_logit"
include { DEMO_LOGIT_PREPROCESS }   from "${method_dir}/preprocess"
include { DEMO_LOGIT_TRAIN }        from "${method_dir}/train"
include { DEMO_LOGIT_PREDICT }      from "${method_dir}/predict"
// This one could be optional and not implement for now
// so make sure to leave it comment out
// include { DEMO_LOGIT_SELECT_FEATURE }      from "${method_dir}}/select_feature" 
  ...
  main:
    // This if here is just to quickly disable a run of method
      ...
      */
      ...
      DEMO_LOGIT_PREPROCESS ( data_copy )
      // Then join the original copy with actual folds after split
      data_copy.join(  DEMO_LOGIT_PREPROCESS.out.fold_splits, by:0 )
              .multiMap { it ->
      ...
      */
      ...
      DEMO_LOGIT_TRAIN ( train_input )

      // Do some transformation to make a multiMap that has two branches for predict
      DEMO_LOGIT_TRAIN.out.model
                  .join(DEMO_LOGIT_TRAIN.out.test_data, by: [0, 1])
                  .multiMap { it ->
      ...
      */
      ...
      DEMO_LOGIT_PREDICT.out.result_table
                    .groupTuple(by: 2)
